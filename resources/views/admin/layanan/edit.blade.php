...
@section('content')
    ...
    <div class="flex items-center gap-2 text-xs text-slate-400 mb-5">
        ...
        <span class="text-slate-600 font-medium">Edit — {{ $layanan->nama_layanan }}</span>
    </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
                    ...
                    <div class="p-5 space-y-4">
                        ...
                        <div>
                            ...
                            <select name="kos_id"
                                ...
                                @foreach ($kos as $k)
                                    <option value="{{ $k->id }}" @selected(old('kos_id', $layanan->kos_id) == $k->id)>
                                        {{ $k->nama_kos }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            ...
                            <textarea name="deskripsi" rows="3"
                                ...
                                class="w-full border border-slate-200 rounded-lg px-3 py-2.5 text-sm text-slate-700 bg-slate-50 focus:outline-none focus:ring-2 focus:ring-blue-400 focus:border-transparent transition resize-none">{{ old('deskripsi', $layanan->deskripsi) }}</textarea>
                        </div>
            ...
            <div class="space-y-5">
                ...
                <div class="bg-white rounded-xl border border-red-100 shadow-sm overflow-hidden">
                    <div class="px-5 py-4 border-b border-red-50 flex items-center gap-2.5">
                        <div class="w-7 h-7 rounded-lg bg-red-50 flex items-center justify-center">
                            <i class="fa-solid fa-triangle-exclamation text-red-400 text-xs"></i>
                        </div>
                        <h2 class="font-bold text-red-400 text-sm">Danger Zone</h2>
                    </div>
                    <div class="p-5">
                        <form action="{{ route('admin.layanan.destroy', $layanan->id) }}" method="POST"
                            ...
                            @csrf @method('DELETE')
                            <button type="submit"
                                class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-red-50 hover:bg-red-100 text-red-500 text-sm font-semibold rounded-lg border border-red-100 transition-colors">
