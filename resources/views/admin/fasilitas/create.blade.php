...
@section('content')
    ...
    <form method="POST" action="{{ route('admin.fasilitas.store') }}" class="space-y-4">
        @csrf

        <div>
            <label class="block mb-1">Nama Fasilitas</label>
            <input type="text" name="nama_fasilitas" class="w-full border rounded p-2" required>
        </div>

        <div>
            <label>Icon (cukup: tv / wifi / bed)</label>

            <input type="text" id="iconInput" name="icon" class="w-full border p-2" placeholder="contoh: tv">
