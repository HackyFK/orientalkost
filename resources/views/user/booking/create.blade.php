...
<div class="max-w-3xl mx-auto pt-28 pb-16 px-4">
    ...
    <div class="text-center mb-8">
        ...
        <p class="text-sm text-slate-400 mt-1">Isi formulir di bawah untuk melanjutkan pemesanan</p>
    </div>
    ...
    <div class="bg-gradient-to-r from-orange-400 to-orange-600 rounded-2xl p-5 mb-6 flex items-center gap-4 shadow-md shadow-orange-200">
        ...
        <div>
            <p class="text-white font-extrabold text-base leading-tight">{{ $kamar->kos->nama_kos }}</p>
        ...
        <div class="space-y-5">
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                <div class="px-6 py-5 grid md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input name="email" type="email" placeholder="Email Aktif" class="input"
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                <div class="px-6 py-5 space-y-5">
                    ...
                    {{-- Jenis Sewa --}}
                    <div>
                        <label class="block text-xs font-semibold text-slate-500 mb-2">Jenis Sewa</label>
                        <div class="grid grid-cols-3 gap-3">
                            @if (!is_null($kamar->harga_harian))
                                <label class="radio-card" :class="jenisSewa === 'harian' ? 'border-indigo-400 bg-indigo-50 text-indigo-700' : ''">
                                    <input type="radio" name="jenis_sewa" value="harian" x-model="jenisSewa" class="hidden">
                                    <i class="fa-regular fa-sun text-xs"></i> Harian
                                </label>
                            @endif
                            <label class="radio-card" :class="jenisSewa === 'bulanan' ? 'border-indigo-400 bg-indigo-50 text-indigo-700' : ''">
                                <input type="radio" name="jenis_sewa" value="bulanan" x-model="jenisSewa" class="hidden">
                                <i class="fa-regular fa-calendar text-xs"></i> Bulanan
                            </label>
                            <label class="radio-card" :class="jenisSewa === 'tahunan' ? 'border-indigo-400 bg-indigo-50 text-indigo-700' : ''">
                                <input type="radio" name="jenis_sewa" value="tahunan" x-model="jenisSewa" class="hidden">
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <select name="durasi" class="input" x-model.number="durasi" required>
                    ...
                    <div class="bg-blue-50 border border-blue-100 rounded-xl p-4 text-sm space-y-2">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="font-semibold text-slate-700" x-text="tanggalMulaiFormatted"></span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-blue-600 font-medium">Selesai</span>
                            <span class="font-semibold text-slate-700" x-text="tanggalSelesaiFormatted"></span>
                        </div>

                        <template x-if="bookings.length > 0">
                            <div class="pt-2 border-t border-blue-200">
                                <p class="text-xs font-semibold text-blue-600 mb-1.5">📋 Periode sudah terisi:</p>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                <div class="px-6 py-4 border-b border-slate-100 flex items-center justify-between">
                    <div class="flex items-center gap-3">
                        <div class="w-8 h-8 rounded-xl bg-violet-50 flex items-center justify-center">
                            <i class="fa-solid fa-sparkles text-violet-400 text-xs"></i>
                        </div>
                    ...
                    <label class="relative inline-flex items-center cursor-pointer gap-2">
                        <div class="relative">
                            <input type="checkbox" x-model="showLayanan" class="sr-only peer">
                            <div class="w-10 h-5 bg-slate-200 peer-checked:bg-indigo-500 rounded-full transition-colors"></div>
                            <div class="absolute top-0.5 left-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform peer-checked:translate-x-5"></div>
                        </div>
                        <span class="text-xs font-semibold text-slate-500" x-text="showLayanan ? 'Aktif' : 'Nonaktif'"></span>
                    </label>
                </div>
                ...
                <div x-show="showLayanan" x-transition class="px-6 py-5">
                    <div class="space-y-2">
                        @foreach ($layanan as $l)
                            <label class="flex items-center justify-between px-3 py-2.5 bg-slate-50 hover:bg-violet-50 rounded-xl border border-slate-100 hover:border-violet-100 transition-colors cursor-pointer group">
                                <div class="flex items-center gap-2.5">
                                    ...
                                    <span class="text-sm font-medium text-slate-700">{{ $l->nama_layanan }}</span>
                                </div>
                                <span class="text-xs font-bold text-slate-500 bg-white border border-slate-200 group-hover:border-violet-200 group-hover:text-violet-600 px-2.5 py-1 rounded-lg transition-colors">
                ...
                <div x-show="!showLayanan" class="px-6 py-4">
                    <p class="text-xs text-slate-400">Aktifkan toggle untuk memilih layanan tambahan.</p>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden">
                ...
                <div class="px-6 py-5 space-y-3 text-sm">
                    ...
                    <div class="flex justify-between items-center">
                        ...
                        <span class="font-semibold text-slate-700" x-text="durasi + ' ' + satuanDurasi"></span>
                    </div>
                    <div class="flex justify-between items-center">
                        <span class="text-slate-500">Harga per <span x-text="satuanDurasi"></span></span>
                        <span class="font-semibold text-slate-700" x-text="format(hargaPerUnit)"></span>
                    </div>
                    <div class="flex justify-between items-center text-green-600">
                        <span>Diskon</span>
                        <span class="font-semibold">- <span x-text="format(discount)"></span></span>
                    </div>

                    <template x-if="selectedLayanan.length > 0">
                        <div class="pt-2 border-t border-dashed border-slate-100 space-y-1.5">
                            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide">Layanan Dipilih</p>
                            <template x-for="(id, index) in selectedLayanan" :key="id">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="font-semibold text-slate-700" x-text="format(layanan.find(l => l.id == id)?.harga)"></span>
                                </div>
                            </template>
                        </div>
                    </template>
                    ...
                    <div class="pt-3 border-t border-slate-100 flex justify-between items-center">
                        ...
                        <span class="text-lg font-extrabold text-indigo-600" x-text="format(grandTotal)"></span>
                    </div>
            ...
            <button type="submit"
                ...
                <span x-text="isBentrok ? 'Tanggal Tidak Tersedia' : (!inputMulai ? 'Pilih Tanggal Dulu' : 'Booking Sekarang')"></span>
            </button>
    ...
    return {
        ...
        initFlatpickr() {
            ...
            self._fp = flatpickr(el, {
                ...
                onChange(selectedDates, dateStr) { self.inputMulai = dateStr }
            })
        },
        ...
        get minMonth() { return this.getMinMonth() },
        ...
        get tanggalSelesai() {
            ...
            else if (this.jenisSewa === 'bulanan') { d.setMonth(d.getMonth() + this.durasi); d.setDate(d.getDate() - 1) }
            else { d.setFullYear(d.getFullYear() + this.durasi); d.setDate(d.getDate() - 1) }
            return d
        },

        get tanggalMulaiFormatted() {
            return this.tanggalMulai ? this.tanggalMulai.toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }) : '-'
        },
        get tanggalSelesaiFormatted() {
            return this.tanggalSelesai ? this.tanggalSelesai.toLocaleDateString('id-ID', { day: '2-digit', month: 'long', year: 'numeric' }) : '-'
        },
        tglFormat(tgl) {
            return tgl ? new Date(tgl + 'T00:00:00').toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' }) : '-'
        },
        ...
        get subtotal() { return this.hargaPerUnit * this.durasi },
        get layananTotal() {
        ...
        get grandTotal() { return this.subtotal - this.discount + this.layananTotal },
        get satuanDurasi() {
        ...
        get durasiOptions() {
            if (this.jenisSewa === 'harian') return Array.from({ length: 30 }, (_, i) => i + 1)
            if (this.jenisSewa === 'bulanan') return Array.from({ length: 11 }, (_, i) => i + 1)
        ...
        get discount() {
            ...
            if (!promo) return 0
            return promo.type === 'percent' ? subtotal * promo.value / 100 : promo.value
        },
        format(val) {
